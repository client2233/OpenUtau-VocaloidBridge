...
以下步骤**假设**用户已经有权使用一个满足 [API 兼容要求](BACKEND_PROTOCOL.md) 的 Python 接口包，以及能正常运行且授权有效的编辑器（Linux 使用 Wine，Windows 原生运行）和传统中文或日语声库。项目不提供这些条件的获取途径，也不认定购买编辑器就包含第三方接口调用许可。
...
7. 中文声库每个音符输入一个 ASCII 拼音音节，例如 `ni3`、`hao3`。日语声库（`lang_id=0`）可用假名或罗马字，例如 `さ`、`sa`、`きゃ`、`kya`，支持促音和元音长音。独立歌词 `ー` 必须紧接以元音结束的音符，不能用于首音或休止之后。其他语言不会自动按日语解析；用户可通过请求协议显式提供 `phoneme`。连音音符填 `-`，延续前一个字而不重新咬字；必须紧接前一个发声音符，不能用于首音或休止之后。然后编辑音高、播放或导出。
...
日语转换由用户提供的改动整合，贡献者报告过传统日语声库测试；维护者未独立验证完整日语后端链路或所有音素的发音质量。转换代码见 `bridge/japanese.py`，支持平假名、片假名和罗马字；转换模块也接受半角片假名，但 OpenUtau 歌词表建议使用全角假名。不转换汉字。日语声库使用单独的歌词表，中文声库保留原拼音表。已有标准描述可用“安装 / 更新声库描述”升级并备份；自定义表不会被覆盖。
...
```sh
python3 bridge/japanese.py
python3 bridge/test_japanese.py
...
桥接服务使用常驻 Wine 工作进程，在连续请求间复用已初始化的合成管理器；配置变更、异常或超时后会关闭旧进程，下次请求重新初始化。停止桥接或退出 OpenUtau 时服务会清理自己的工作进程。每句完成后的 `PASS render` 日志包含请求耗时。
